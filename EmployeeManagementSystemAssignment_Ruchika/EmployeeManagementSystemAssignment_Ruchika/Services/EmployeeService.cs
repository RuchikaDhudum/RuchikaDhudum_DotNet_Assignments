...
namespace EmployeeManagementSystemAssignment_Ruchika.Services
{
    public class EmployeeService : IEmployeeService
    {
        ...
        //filter to get employee additional details by passing filter as employee basic details UId
        public async Task<EmployeeADModel> GetEmployeeADByEmployeeBDUId(string employeeBDUId)
        {
            var employee = await GetAllEmployeeAD();

            var filter = employee.Find(x => x.EmployeeBDUId == employeeBDUId);
            return filter;
        }
        ...
        public async Task<EmployeeFilterCriteria> GetAllEmployeeByPagination(EmployeeFilterCriteria employeeFilterCriteria)
        {
            ...
            foreach(var emp in filteredRecords)
            {
        ...
        {
            EmployeeADFilterCriteria response = new EmployeeADFilterCriteria();
            //filter = Status
            var checkFilter = employeeADFilterCriteria.Filters.Any(x => x.FieldName == "status");
            var status = "";
            if (checkFilter)
            {
                status = employeeADFilterCriteria.Filters.Find(x => x.FieldName == "status").FieldValue;
            }

            var employees = await GetAllEmployeeAD();

            var filteredRecords = employees.FindAll(x => x.Status == status);

            response.totalCount = employees.Count;
            response.page = employeeADFilterCriteria.page;
            response.pageSize = employeeADFilterCriteria.pageSize;

            var skip = employeeADFilterCriteria.pageSize * (employeeADFilterCriteria.page - 1);

            filteredRecords = filteredRecords.Skip(skip).Take(employeeADFilterCriteria.pageSize).ToList();
        ...
        public async Task<VisitorModel> AddVisitorByMakePostRequest(VisitorModel visitorModel)
        {
            var SerializeObject = JsonConvert.SerializeObject(visitorModel);
            var requestObject = await HttpClientHelper.MakePostRequest(Credentials.VisitorUrl, Credentials.AddVisitorEndPoint, SerializeObject);
            var responseObject = JsonConvert.DeserializeObject<VisitorModel>(requestObject);
            return responseObject;
        }

        public async Task<List<VisitorModel>> GetVisitorByMakeGetRequest()
        {
            var requestObj = await HttpClientHelper.MakeGetRequest(Credentials.VisitorUrl, Credentials.GetVisitorEndPoint);
            var responseObj = JsonConvert.DeserializeObject<List<VisitorModel>>(requestObj);
            return responseObj;
        }
